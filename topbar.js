...
`;
  ...
  function getFoodProgress() {
    ...
    // Same rule as Calendrier/alimentation.html's isRestDayToday(): a day
    // placed in po_day_overrides_v1 wins, an untouched day defaults to
    // rest. Fitness's rotation formula is no longer consulted here — it
    // kept disagreeing with what Calendrier actually shows.
    try {
      const overrides = JSON.parse(localStorage.getItem('po_day_overrides_v1'));
      const dk = calendarDateKey();
      const override = overrides && overrides[dk];
      const restToday = override ? /repos|rest/i.test(override) : true;
